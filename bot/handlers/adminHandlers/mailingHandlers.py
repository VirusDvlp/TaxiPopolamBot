...
from keyboards import get_add_bt_kb, get_cancel_kb, get_main_admin_kb
    ...
    async with state.proxy() as data:
        ...
        for user in users:
            try:
                ...
                continue
    await callback.message.answer('Рассылка\tуспешно\tсоздана', reply_markup=get_main_admin_kb())
    await MainAdminFSM.adminState.set()
    await callback.answer()

def register_mailing_hadnlers(dp: Dispatcher) -> None:
    dp.register_message_handler(make_mailing, Text('СОЗДАТЬ РАССЫЛКУ ПОЛЬЗОВАТЕЛЯМ✉️'), state=MainAdminFSM.adminState)
